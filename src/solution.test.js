describe('Домашнее задание ШРИ по теме “Асинхронность”', function() {
  ...
  describe('Вариант 4. Посчитать площадь треугольника (promise)', function() {
    it('возвращает площать треугольника 1', function() {
      const x1 = 0, y1 = 0, x2 = 0, y2 = 0, x3 = 0, y3 = 0;

      return calcAreaOfTriangle_promise(x1, y1, x2, y2, x3, y3)
        .then(area => { chai.assert.equal(area, 0) });
    });
    it('возвращает площать треугольника 2', function() {
      const x1 = 1, y1 = 1, x2 = 2, y2 = 5, x3 = 5, y3 = 3;

      return calcAreaOfTriangle_promise(x1, y1, x2, y2, x3, y3)
        .then(area => { chai.assert.equal(area, 7) });
    });
    it('возвращает площать треугольника 3', function() {
      const x1 = 0, y1 = 0, x2 = 10, y2 = 3, x3 = 4, y3 = 8;

      return calcAreaOfTriangle_promise(x1, y1, x2, y2, x3, y3)
        .then(area => { chai.assert.equal(area, 34) });
    });
  });
  describe('Вариант 4. Посчитать площадь треугольника (async)', function() {
    it('возвращает площать треугольника 1', function() {
      const x1 = 0, y1 = 0, x2 = 0, y2 = 0, x3 = 0, y3 = 0;

      return calcAreaOfTriangle_async(x1, y1, x2, y2, x3, y3)
        .then(area => { chai.assert.equal(area, 0) });
    });
    it('возвращает площать треугольника 2', function() {
      const x1 = 1, y1 = 1, x2 = 2, y2 = 5, x3 = 5, y3 = 3;
